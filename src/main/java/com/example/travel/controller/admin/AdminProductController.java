...
public class AdminProductController {
    ...
    public String createAdminProduct(InsertProductRequest request, Principal principal, RedirectAttributes rttr) {
        ...
        return "redirect:/seller/product/" + product.getProductId() + "/show";
    }

    // admin_product_show 상품 보기 페이지
    @GetMapping("/seller/product/{productId}/show")
    ...
    @GetMapping("/seller/product/index")                           // default 페이지, 한 페이지 게시글 수, 정렬기준 컬럼, 정렬순서
    public String indexAdminProduct(Model model, @PageableDefault(page = 0, size = 10, sort = "productId", direction = Sort.Direction.DESC) Pageable pageable,
                                    ...
                                    @RequestParam(defaultValue = "") String subCategory, @RequestParam(defaultValue = "") String searchKeyword) {
        ...
        Page<Product> productList = productService.productList(status, mainCategory, subCategory, searchKeyword, pageable);
        ...
        if(isDel != null && isDel == 1) { // 체크해서 숨김 했을 때 팝업창
            model.addAttribute("msg", "상품이 숨김처리 되었습니다.");
        }
        if(isDel != null && isDel == 0) { // 체크 안하고 숨김 눌렀을 때 팝업창
    ...
    // 상품 숨김
    @GetMapping("/seller/product/{productId}/delete")
    public String deleteAdminNotice(@PathVariable Long productId, RedirectAttributes rttr, Integer page, String status, String mainCategory, String subCategory, String searchKeyword) throws UnsupportedEncodingException {
        ...
        return "redirect:/seller/product/index?page=" + page + "&status=" + encodeStatus + "&mainCategory=" + encodeMainCategory + "&subCategory=" + encodeSubCategory + "&searchKeyword=" + encodeSearchKeyword;
    }
